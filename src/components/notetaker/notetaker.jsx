    ...
    return (
        <>
            <div className="doc-options">
                ...
                <div className="main">
                    <button onClick={clearwritingarea} style={{backgroundColor:"#ed5e68", display:"flex",justifyContent:"center",alignItems:"center"}}>Clear</button>
                    <button onClick={downloadfile} style={{backgroundColor:"#a6ff4d", color:"#578723"}}>Download</button>
                </div>
            </div>
        </>
    );
...
function Notetaker({ darkMode }) {
    return (
        <div className={`notetaker-container ${darkMode ? 'dark-mode' : ''}`}>
            <EditorProvider slotBefore={<MenuBar />} extensions={extensions} content="Welcome to QuikNotes, Happy Writing !!! (Click on the clear button to reset the page)">
                <EditorContent />
            </EditorProvider>
        </div>
    );
